        ...
        try
        {
            ...
            while (colIterator.hasNext())
            {
             ...
             // Now add the row data. Note that we skip the 3rd column as
                ...
                // czq add.
                csv.setTableColumnSize(row.size());
                logger.info("---@---Table columns is " + row.size() + "---@---");
                logger.info("---@---csv table columns is " + csv.getTableColumnSize() + "---@---");
                ...
                for (int i = 0; i < row.size(); i++)
                {
                    ...
                    logger.info("row.get(i):" + row.get(i));
                    ...
                    //value类似[B@1a16fc7：Type=12 
                    logger.info("columnVal:" + columnVal.toString());
                    logger.info("rawValue before getCsvString: " + rawValue + " Type=" +  columnSpec.getType() + "isBlob=" + columnSpec.isBlob() + rawValue.getClass());
                    logger.info(rawValue.toString());
                    //if(columnSpec.getType() == 12 ) {
                    //    columnSpec.setBlob(true);
                    //}
                    logger.info("Blob:" + columnSpec.isBlob());
                    String value = getCsvString(rawValue, columnSpec);
                    
                    logger.info("rawValue after getCsvString:" + value);
